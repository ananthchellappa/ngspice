...
/* Remove a variable.
   ...
   the same block a second time.  doc/codex/issues/0067. */
...
void cp_remvar(char *varname)
{
    ...
    case US_DONTRECORD:
        /* 'curplot' 'curplotname' 'curplottitle' 'curplotdate'.  cp_enqvar()
           ...
           the name as a number.  doc/codex/issues/0067 criteria 4 and 5. */
        fprintf(cp_err, "Error: %s cannot be unset.\n", v->va_name);
        break;

    case US_READONLY:
        /* 'plots' and 'curcasemode', both computed in cp_enqvar().  Refusing
           is the whole of the arm; the node stays where it was found. */
